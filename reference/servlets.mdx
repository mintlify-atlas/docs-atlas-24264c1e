---
title: "Servlet reference: all HTTP endpoints in TEI"
...
Authenticates a teacher. Identical flow to `/loginAdmin` but uses role `"MAESTRO"` and redirects to `view/principal/menuProfesor.jsp`.
...
| Parámetro | Tipo | Descripción |
|---|---|---|
| `id_grade` | int | ID del nivel académico (`grade.id_grade`) |
| `level_group` | int | Número de nivel dentro del grado |
| `id_category_group` | int | ID de la categoría (`category.id_category`) |
| `classroom_group` | String | Identificador del salón |
...
Student self-registration endpoint (limited use — not exposed in the main admin UI). Accepts the same parameters as `/addStudent`. Used for public enrollment flows.
...
| Parámetro | Tipo | Descripción |
|---|---|---|
| `id_user` | int | Identificador del usuario a actualizar |
| `nom_user` | String | Nuevo nombre de usuario |
| `password1` | String | Nueva contraseña |
| `password2` | String | Confirmación de nueva contraseña |
| `rango` | String | Nuevo rol del usuario |
...
Redirects to `listaAlumnos.jsp` if the session role is `ADMINISTRADOR`, or to `asignarCalificaciones.jsp` if is `MAESTRO`.
...
| Parámetro | Tipo | Descripción |
|---|---|---|
| `id_pay` | int | ID de la entrada del calendario |
| `month` | String | Nombre del mes |
| `description_pay` | String | Descripción del concepto |
| `cost_pay` | double | Costo en pesos |
| `period_pay` | String | `Periodo A`, `Periodo B`, o `Cualquiera` |
| `deadline_pay` | String | Fecha límite (`YYYY-MM-DD`) o vacío para `NULL` |
