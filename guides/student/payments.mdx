---
title: "Payment tracking and calendar for TEI students"
sidebarTitle: "Payments"
description: "The TEI tracks up to 8 payment slots per student: an enrollment fee plus 7 monthly installments. Administrators mark payments and update the semester calendar."
---
...
2. **Tabla de mensualidades** — one row per calendar slot from `pay_simbology`, filtered for the current period via `base.obtenerCalendarioFiltrado()`. Each row shows:
   ...
   | **Estatus de Referencia** | "PAGADO" (verde) o "PENDIENTE" (rojo), driven by the JS file `seguimientoPagoJScript.js` |

The color coding uses inline styles: green `rgba(63, 166, 83, 0.5)` for paid, red `rgba(245, 39, 39, 0.5)` for pending.
