---
title: "Session management and timeout handling in TEI"
sidebarTitle: "Sessions"
description: "TEI sessions expire after 30 minutes of inactivity. The system detects expired sessions across all JSPs and redirects users to the session-expired page."
---
...
if (sesion != null) {
    ...
    // Determine role-specific redirect URL from rango…
    sesion.invalidate();
}
response.sendRedirect(url); // role login page, or URL_INDEX as fallback
```

The role-to-URL mapping used after invalidation:

| Rango | Redirect destino |
|---|---|
| `ESTUDIANTE` | `Constantes.VentanasJSP.URL_LOGIN_ALUMNO` |
| `ADMINISTRADOR` | `Constantes.VentanasJSP.URL_LOGIN_ADMIN` |
| `PROFESOR` | `Constantes.VentanasJSP.URL_LOGIN_TEACHER` |
| *(no session / null rango)* | `Constantes.VentanasJSP.URL_SESION_EXPIRADA` |
