...
The system includes a database backup feature that relies on `mysqldump.exe`, which must be placed at the following path inside the deployed web application:

```
src/main/webapp/WEB-INF/mysqldump.exe
```

This path is resolved at runtime via the `Constantes.ARCHIVO_MYSQLDUMP` constant (`/tallerDeInglesUAEM/WEB-INF/mysqldump.exe`).
